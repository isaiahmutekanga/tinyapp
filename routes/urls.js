...
const getUrlsForUser = require("../helpers/urls-for-user").urlsForUser;
...
router.post("/", (req, res) => {
  ...
  if (req.session["user_id"]) {
    ...
    res.end(
      "<html><head><title>TinyApp: Error</title></head><body>Unfortunately, you can't make a new short URL unless you are logged in. <a href='/register'>Register</a> or <a href='/login'>login</a>.</body></html>\n"
    );
  }
});
...
router.post("/:id", (req, res) => {
  ...
  if (
    ...
    res.end(
      "<html><head><title>TinyApp: Error</title></head><body>Unfortunately, you can only change URLs you created. <a href='/register'>Register</a> or <a href='/login'>login</a>.</body></html>\n"
    );
  }
});
...
router.post("/:id/delete", (req, res) => {
  ...
  if (urlDatabase[req.params.id].userID === users[req.session["user_id"]].id) {
    ...
    res.end(
      "<html><head><title>TinyApp: Error</title></head><body>I'm not sure what you're doing here, but please stop trying to delete things you don't own! Please <a href='/register'>register</a> or <a href='/login'>login</a>.</body></html>\n"
    );
  }
});
