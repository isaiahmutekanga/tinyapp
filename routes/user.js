...
const getRandomString =
  require("../helpers/random-string").generateRandomString;
...
router.post("/register", (req, res) => {
  for (let userId in users) {
    if (users.hasOwnProperty(userId)) {
      ...
      if (req.body.email === users[userId].email) {
        return res.end(
          "<html><head><title>TinyApp: Error</title></head><body>Email address already in database. <a href='/user/register'>Register</a> or <a href='/user/login'>login</a>.</body></html>\n"
        );
      }
    }
  }
  ...
  if (req.body.email === "") {
    return res.end(
      "<html><head><title>TinyApp: Error</title></head><body>Missing email address from registration form. <a href='/user/register'>Register</a> or <a href='/user/login'>login</a>.</body></html>\n"
    );
    // If password field is empty, error
  } else if (req.body.password === "") {
    return res.end(
      "<html><head><title>TinyApp: Error</title></head><body>Missing password from registration form. <a href='/user/register'>Register</a> or <a href='/user/login'>login</a>.</body></html>\n"
    );
...
router.post("/login", (req, res) => {
  ...
  if (existingEmails.indexOf(req.body.email) === -1) {
    return res.end(
      "<html><head><title>TinyApp: Error</title></head><body>Invalid username or password. <a href='/user/register'>Register</a> or <a href='/user/login'>login</a>.</body></html>\n"
    );
